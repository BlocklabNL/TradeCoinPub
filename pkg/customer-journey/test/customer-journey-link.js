...
contract('CustomerJourney', async (accounts) => {
    ...
    it('link assets', async () => {
        let notaryOwner = await this.notary.owner();
        let journeyOwner = await this.journey.owner();

        await this.notary.registerApplication(app, appDescription, {from: notaryOwner});

        // register a bunch of assets with the notary
        let hashes = generateHashes(10);
        for (let i = 0; i < hashes.length; i++) {
            let tx = await this.notary.registerAsset(network, hashes[i], app, pathSpec, expiresAt, {from: owner});
