...
function CompAddInformationHandler() {
  const [address, setAddress] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);

  async function compAddInformationHandler() {
    if (!address) return;
    ...
    <div>
      ...
      <Container>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Add Information Handler</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
        ...
        <Row>
          <Col><Form onSubmit={submitHandler}>
            <Form.Group className="mb-3" controlId="formTokenId">
              <Form.Label>Product Information Handler</Form.Label>
...
export default CompAddInformationHandler;
