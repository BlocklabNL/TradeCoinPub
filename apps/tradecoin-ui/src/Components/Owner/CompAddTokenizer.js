...
function CompAddTokenizer() {
  const [address, setAddress] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);

  async function compAddTokenizer() {
    if (!address) return;
...
<Container>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active>Add Tokenizer</Breadcrumb.Item>
        </Breadcrumb>
      </Container>
        ...
        <Row>
          <Col><Form onSubmit={submitHandler}>
            <Form.Group className="mb-3" controlId="formTokenId">
              <Form.Label>Product Tokenizer</Form.Label>
...
export default CompAddTokenizer;
